...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a deterministic Asperitas evidence pack from MVP-003 retrieval results.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    retrieved = search_chunks_mvp003(args.query, chunks, records, limit=args.top_k, include_explanations=True)
    pack = build_evidence_pack(args.query, retrieved, top_k=args.top_k)
    print(json.dumps(pack.to_json(), ensure_ascii=False, indent=2))
    return 0
